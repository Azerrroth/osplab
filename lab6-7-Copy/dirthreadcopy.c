...
void copy_folder(char* source, char* destination)
{
	...
	while(filename = readdir(dp))
	{
		...
		{
			if(!endwith(file_source, '.'))	copy_folder(file_source, file_dest);
		}
		else
		{
			fd_src = open(file_source, O_RDONLY);
			file_size = lseek(fd_src, 0, SEEK_END);
			thread_num = file_size/(10*1024*1024);// size over 10 MB, use multithread
...
int main(int argc,char *argv[])
{
	...
	else
	{
		...
		{	
			...
			copy_file_thread(argv[1], argv[2]);
		}
	}
	return 0;
}
